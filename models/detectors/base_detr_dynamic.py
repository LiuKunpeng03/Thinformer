...
class DetectionTransformerDynamic(BaseDetector, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.lambda_top_k = lambda_top_k
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        """
        img_feats, k = self.extract_feat(batch_inputs)
        ...
        '''
        Following codes are added to calculate L_cons
        '''
        img_shapes = [sample.ori_shape for sample in batch_data_samples]
        train_mlp = True # self.backbone.stages[0].train_mlp
        if train_mlp:
            # calculate the loss of top_k
            results_list = self.bbox_head.predict(
                hidden_states=head_inputs_dict['hidden_states'],
                references=head_inputs_dict['references'],
                rescale=True,
                batch_data_samples=batch_data_samples)
            value_ratio = self.value_ratio_calculator(results_list, img_shapes)
            for i, top_k_ratio in enumerate(k):
                loss_top_k_i = self.lambda_top_k * F.mse_loss(
                    top_k_ratio,
                    torch.tensor(value_ratio, device=batch_inputs.device, dtype=torch.float32)
                )
                losses[f'loss_top_k_{i}'] = loss_top_k_i

        return losses
    
    def value_ratio_calculator(self, 
                               results_list: InstanceList,
                               img_shapes: list,
                               score_thresh: float = 0.3) -> float:
        """Calculate the value ratio of the bboxes, considering overlaps (efficient for large images).
        Args:
            results_list (list[:obj:`InstanceData`]): Detection results of each image.
            img_shapes (list): List of (H, W) for each image.
            score_thresh (float): Score threshold for filtering bboxes.
        Returns:
            float: The average value ratio of the bboxes (union area / img area).
        """
        ...
        for i, result in enumerate(results_list):
            ...
            if bboxes is None or bboxes.numel() == 0 or scores is None:
                ratios.append(0.0)
                continue
            ...
            polys = [shapely_box(*box.tolist()) for box in bboxes.cpu()]
            ...
            if img_area <= 0:
                ...
                ratios.append(union_area / img_area)
        if len(ratios) == 0:
            return 0.0
    ...
    def predict(self,
        ...
        """
        img_feats, ks = self.extract_feat(batch_inputs)
        head_inputs_dict = self.forward_transformer(img_feats,
                                                    batch_data_samples)
        results_list = self.bbox_head.predict(
            **head_inputs_dict,
        ...
        # '''
        # Following lines are added to save the top-ks
        # '''
        
        # img_shapes = [sample.ori_shape for sample in batch_data_samples]

        # value_ratio = self.value_ratio_calculator(results_list, img_shapes)
        # top_k_ratios_list = [x.item() for x in ks]

        # save_dir = '/data/liukunpeng/newtopklogs'
        # os.makedirs(save_dir, exist_ok=True)
        # save_path = os.path.join(save_dir, 'new_loss.json')
        # data = {
        #     'top_k_ratios': top_k_ratios_list,
        ...
        logger.info(f'use_mlp: {self.backbone.stages[0].use_dynamic_topk}')
        """
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        """
        x, k = self.backbone(batch_inputs)
        if self.with_neck:
            x = self.neck(x)
        return x, k
